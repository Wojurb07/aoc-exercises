...
# they only have to worry about the length they need to order, which they would again like to be exact.
...
def box_wrapping(length, width, height):
    lst = [length*width, width*height, length*height]
    card = min(lst)

    surface = 2*length*width + 2*width*height + 2*height*length
    total_paper = surface + card
    return total_paper
...
def bow_ribbon(length, width, height):
    ...
    ribbon_wrap = lst[0]*2 + lst[1]*2
    print(ribbon_wrap)
    ribbon_bow = lst[0]*lst[1]*lst[2]
    return ribbon_wrap + ribbon_bow

fhand = open("input-2.txt")
total = 0
ribbon = 0
...
for line in fhand:
    ...
    w = int(measures[1])
    h = int(measures[2])
    total += box_wrapping(l, w, h)
    ribbon += bow_ribbon(l, w, h)
print(total)
print(ribbon)
...
# You arrive at the secret entrance to the North Pole base ready to start decorating. Unfortunately, the password seems to have been
# changed, so you can't get in. A document taped to the wall helpfully explains:
...
# taught you that the safe is actually a decoy. The actual password is the number of times the dial is left pointing at 0 after any rotation in the sequence.
...
# fhand = open('2025/input-1.txt')
# counter = 0
# index = 50

# for rotation in fhand:
#     rotation = rotation.rstrip()
#     direction = rotation[0]
#     distance = int(rotation[1:])
#     if direction == "R":
#         if index + distance > 99:
#             index = (index + distance) % 100
#         else:
#             index+=distance
#     elif direction == "L":
#         if index - distance < 0:
#             index = (index - distance) % 100 # index = 50 - 100 % 50
#         else:
#             index-=distance
# #    print(index)
#     if index == 0:
...
# You remember from the training seminar that "method 0x434C49434B" means you're actually supposed to count the number of times any
# click causes the dial to point at 0, regardless of whether it happens during a rotation or at the end of one.
...
for rotation in fhand:
    ...
    if direction == "R":
        if index + distance > 99: # 40 + 1000 -> 1040 -> +10zeros + 40
            zeros = (index + distance) // 100
            index = (index + distance) % 100
            counter+=zeros
            if index == 0:
                counter+=1
